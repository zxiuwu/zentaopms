function addField(obj)
{
    ...
    tbody.find('tr:last').find('td').eq(3).find('input:checkbox[name^="showAlone"]').attr('name', 'showAlone[' + tbody.find('tr:last').index() + ']').attr('disabled', 'disabled');
    tbody.find('tr:last').find('td').eq(3).find('select').attr('name', 'contrast[' + tbody.find('tr:last').index() + ']').attr('disabled', 'disabled');
...
$(function()
{
    ...
    {
        $.post(createLink('report', 'ajaxCheckVar', 'reportID=' + reportID + '&type=' + type), {'sql': $('.sqlForm #sql').val()}, function(data)
        {
            if(data)
            {
                data = eval('(' + data + ')');
