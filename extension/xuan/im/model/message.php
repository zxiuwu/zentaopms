...
class imMessage extends model
{
    ...
    public function create($messageList = array(), $userID = 0)
    {
        ...
        foreach($messageList as $message)
        {
            ...
            elseif(!$msg)
            {
                ...
                $msgIndex = $this->dao->select('`lastMessageIndex` + 1 as msgIndex')->from(TABLE_IM_CHAT)->where('gid')->eq($message->cgid)->fetch('msgIndex');
    ...
    public function retract($gid = '', $byAdmin = false, $deletedBy = 0)
    {
        ...
        if($archiveDate != null) return array();
    ...
    public function createDetachedConferenceEnableNotify()
    {
        ...
        $userModel = $this->im->user;

        $allUsers = $userModel->getList();
        $allUsers = array_keys($allUsers);

        $newClientUsers = array_filter($allUsers, function($user) use($userModel)
        {
            return $userModel->isDeviceVersionGe($user, '7.2.beta');
        });
