<?php
/**
 * The dynamic browse mobile view file of project module of ZenTaoPMS.
 *
...
<nav id='menu' class='menu nav affix dock-top canvas'>
  <?php
  echo html::a(inlink('dynamic', "projectID=$projectID&type=today"),      $lang->action->dynamic->today);
  echo html::a(inlink('dynamic', "projectID=$projectID&type=yesterday"),  $lang->action->dynamic->yesterday);
  echo html::a(inlink('dynamic', "projectID=$projectID&type=twodaysago"), $lang->action->dynamic->twoDaysAgo);
  echo html::a(inlink('dynamic', "projectID=$projectID&type=thisweek"),   $lang->action->dynamic->thisWeek);
  echo html::a(inlink('dynamic', "projectID=$projectID&type=lastweek"),   $lang->action->dynamic->lastWeek);
  echo html::a(inlink('dynamic', "projectID=$projectID&type=thismonth"),  $lang->action->dynamic->thisMonth);
  echo html::a(inlink('dynamic', "projectID=$projectID&type=lastmonth"),  $lang->action->dynamic->lastMonth);
  echo html::a(inlink('dynamic', "projectID=$projectID&type=all"),        $lang->action->dynamic->all);
  ?>
...
<section id='page' class='section list-with-actions list-with-pager'>
  <?php $refreshUrl = $this->createLink('project', 'dynamic', "projectID=$projectID&type=$type&param=$param&orderBy=$orderBy&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}&pageID={$pager->pageID}");?>
...
<div class='list sort-panel hidden affix enter-from-bottom layer' id='sortPanel'>
  <?php
  $vars = "projectID=$projectID&type=$type&param=$param&orderBy=%s&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}&pageID={$pager->pageID}";
